...
public class Passant extends Piece {
  ...
  public Passant(Chess chess, Player player, int x, int y) {
    super(chess, player, x, y);
  }

  public void moveConstructor() {
    Piece piece;
    if ((piece = this.getChess().getPieceAtPosition(this.getX(), this.getY()+this.playerSideModifier())) == null) {
    ...
    if ((piece = this.getChess().getPieceAtPosition(this.getX(), this.getY() + 2 * this.playerSideModifier())) == null) {
      if (!this.hasPieceMoved()) {
        this.addMove(new EnPassantMove(this, this.getX(), this.getY() + 2 * this.playerSideModifier()));
      }
    }

    if ((piece = this.getChess().getPieceAtPosition(this.getX()-1, this.getY()+this.playerSideModifier())) != null) {
      if (piece.getPlayer().getColor() != this.getPlayer().getColor()) {
        this.addMove(new Attack(this, piece, this.getX()-1, this.getY() + this.playerSideModifier()));
      }
    } else if ((piece = this.getChess().getPieceAtPosition(this.getX()+1, this.getY() + this.playerSideModifier())) != null) {
      if (piece.getPlayer().getColor() != this.getPlayer().getColor()) {
        this.addMove(new Attack(this, piece, this.getX()+1, this.getY() + this.playerSideModifier()));
      }
    }
    ...
    //En Passant.
    if ((piece = this.getChess().getPieceAtPosition(this.getX()-1, this.getY())) != null) {
      if (piece.getPlayer().getColor() != this.getPlayer().getColor() && this.vulnerableToEnPassant()) {
        this.addMove(new EnPassantAttack(this, piece, this.getX()-1, this.getY() + this.playerSideModifier()));
      }
    } else if ((piece = this.getChess().getPieceAtPosition(this.getX() + 1, this.getY() + this.playerSideModifier())) != null) {
      if (piece.getPlayer().getColor() != this.getPlayer().getColor() && this.vulnerableToEnPassant()) {
        this.addMove(new EnPassantAttack(this, piece, this.getX() + 1, this.getY() + this.playerSideModifier()));
